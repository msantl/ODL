...
public class Mjerenje1 implements IListenDataPacket {
    ...
    Match createMatch(InetAddress src, InetAddress dst, byte transportProto, short srcPort, short dstPort) {
        Match match = new Match();

        /* match udp or tcp IPv4 ethernet packets with matching IP addresses and transport ports */
        try {
            match.setField(MatchType.DL_TYPE, EtherTypes.IPv4.shortValue());
            match.setField(MatchType.NW_DST, dst);
            match.setField(MatchType.NW_SRC, src);
            match.setField(MatchType.NW_PROTO, transportProto);
    ...
    void installFlow(Node node, NodeConnector nodeConnector,
            InetAddress src, InetAddress dst, byte transportProto,
            short srcPort, short dstPort, boolean setContollerToListen) {
        ...
        if (setContollerToListen) {
            /*System.out.println("Setting Controller to listen for packets on " +
                    node.toString() + " to " + dst.getHostAddress() + ":" + dstPort);*/
            actions.add(new Controller());
        }

        Match match = createMatch(src, dst, transportProto, srcPort, dstPort);
        Flow flow = new Flow(match, actions);
        flow.setPriority(PRIORITY);
        flow.setIdleTimeout(IDLE_TIMEOUT);

        String policyName = src.getHostAddress() + "/" + dst.getHostAddress() + "/" + srcPort + "/" + dstPort;
        ...
        System.out.print(" " + srcPort + " " + dstPort);*/
    ...
    boolean createNewFlow(InetAddress source, InetAddress destination, byte transportProto,
        ...
        if (flows != null) {
            Match m1 = createMatch(source, destination, transportProto, srcPort, dstPort);
            Match m2 = createMatch(destination, source, transportProto, dstPort, srcPort);
            for (FlowEntry flow: flows) {
                if (m1.equals(flow.getFlow().getMatch())) {
                    needToInstallFlow = false;
                    break;
                }
                if (m2.equals(flow.getFlow().getMatch())) {
                    needToInstallFlow = false;
                    break;
                }
            }
        ...
        if (needToInstallFlow) {
            ...
            /* set flow for source */
            installFlow(srcNode, srcNodeConnector, destination, source, transportProto, dstPort, srcPort, listen);

            /* set flow for destination */
            installFlow(dstNode, dstNodeConnector, source, destination, transportProto, srcPort, dstPort, listen);

            /* install flows along the path */
            for (Edge e: path) {
                installFlow(e.getHeadNodeConnector().getNode(),
                        e.getHeadNodeConnector(), source, destination,
                        transportProto, srcPort, dstPort,
                        false);

                installFlow(e.getTailNodeConnector().getNode(),
                        e.getTailNodeConnector(), destination, source,
                        transportProto, dstPort, srcPort,
                        false);
            }
    ...
    public PacketResult receiveDataPacket(RawPacket inPkt) {
        /* Extract IP addresses and transport ports from packet data */
        ...
        try {
            if (formattedPak instanceof Ethernet) {
                ...
                if (nextPak instanceof IPv4) {
                    ...
                    Object transportPak = ipPak.getPayload();
                    byte transportProto;

                    /* Extract ports from the transport PDU */
                    if (transportPak instanceof UDP) {
                        srcPort = ((UDP) transportPak).getSourcePort();
                        dstPort = ((UDP) transportPak).getDestinationPort();

                        transportProto = IPProtocols.UDP.byteValue();
            ...
            //System.out.println("Receive data packet exception " + e.toString());
        }
